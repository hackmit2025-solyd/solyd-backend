...
class EntityMatch(BaseModel):
    ...
    matched_entity: Dict[str, Any] = Field(
        ..., description="Matched entity from database"
    )
...
class SearchResponse(BaseModel):
    ...
    entity_mappings: Dict[str, EntityMatch] = Field(
        default_factory=dict, description="Entity mappings used in query"
    )
...
class GraphSearchResponse(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata about the query and results"
    )
...
class ErrorResponse(BaseModel):
    ...
    suggested_query: Optional[str] = Field(
        None, description="Suggested alternative query"
    )
