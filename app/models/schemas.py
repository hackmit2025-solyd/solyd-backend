...
class DocumentUpload(BaseModel):
    ...
    title: Optional[str] = Field(
        None, description="Document title", example="Patient Visit Record"
    )
    content: str = Field(
...
class ExtractionResult(BaseModel):
    ...
    assertions: List[Dict[str, Any]] = Field(
        ...
        example=[
            {"predicate": "HAS_SYMPTOM", "subject_ref": "E567", "object_ref": "fever"}
        ],
    )
...
class EntityResolutionRequest(BaseModel):
    ...
    entity_type: str = Field(
        ..., description="Type of entity to resolve", example="patient"
    )
    entity_data: Dict[str, Any] = Field(
        ..., description="Entity attributes for matching"
    )
...
class EntityResolutionResult(BaseModel):
    ...
    decision: Literal["match", "new", "abstain"] = Field(
...
class GraphQuery(BaseModel):
    ...
    query: str = Field(
        ...
        example="MATCH (p:Patient) RETURN p LIMIT 10",
    )
...
class ChatRequest(BaseModel):
    ...
    message: str = Field(
        ...
        min_length=1,
        example="What symptoms did the patient have?",
    )
...
class ChatResponse(BaseModel):
    ...
    graph_path: Optional[List[Dict[str, Any]]] = Field(
        None, description="Relevant graph traversal path"
    )
    evidence: Optional[List[Dict[str, Any]]] = Field(
        None, description="Supporting evidence from graph"
    )
...
class SubgraphResponse(BaseModel):
    ...
    relationships: List[Dict[str, Any]] = Field(
        ..., description="List of relationships in subgraph"
    )
