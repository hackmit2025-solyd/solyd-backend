...
class TextractService:
    ...
    def process_document_from_s3(
        self, s3_key: str, feature_types: Optional[List[str]] = None
    ) -> Dict[str, Any]:
    ...
    def process_document_bytes(
        self, document_bytes: bytes, feature_types: Optional[List[str]] = None
    ) -> Dict[str, Any]:
    ...
    def _extract_table(self, table_block: Dict, block_map: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if cells:
                ...
                cells.sort(
                    key=lambda x: (x.get("RowIndex", 0), x.get("ColumnIndex", 0))
                )
    ...
    def _get_text_from_relationships(
        self, relationships: List[Dict], block_map: Dict
    ) -> str:
    ...
    def _extract_medical_sections(self, result: Dict) -> Dict:
        ...
        for form_field in result.get("forms", []):
            ...
            if any(
                term in key_lower for term in ["patient", "name", "dob", "mrn", "id"]
            ):
                ...
                term in key_lower for term in ["medication", "drug", "dose", "rx"]
            ):
                medical_sections["medications"].append(form_field)
            elif any(
                term in key_lower for term in ["diagnosis", "dx", "icd", "condition"]
            ):
                medical_sections["diagnoses"].append(form_field)
            elif any(
                term in key_lower for term in ["procedure", "surgery", "operation"]
            ):
        ...
        for table in result.get("tables", []):
            if table["rows"]:
                ...
                if any(
                    "test" in str(cell).lower() or "result" in str(cell).lower()
                    for cell in header
                ):
