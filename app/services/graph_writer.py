...
class GraphWriter:
    ...
    ALLOWED_LABELS = {
        "Patient", "Encounter", "Symptom", "Disease", "Test", "TestResult",
        "Medication", "Procedure", "Clinician", "Guideline", "SourceDocument",
        "Assertion", "OntologyTerm", "ExternalResource", "Chunk"
    }

    # Whitelist of allowed relationship types
    ALLOWED_RELATIONSHIPS = {
        "HAS_ENCOUNTER", "HAS_SYMPTOM", "DIAGNOSED_AS", "PRESCRIBED",
        "ORDERED_TEST", "YIELDED", "TREATED_BY", "REFERRED_TO",
        "ALLERGIC_TO", "CONTRAINDICATED", "PERFORMED", "EVIDENCED_BY",
        "ALIGNS_WITH", "CITES", "EXTRACTED_FROM", "CONFLICTS_WITH",
        "RESOLVES_TO", "PART_OF", "FOLLOWED_BY"
    }

    # Property types that need special handling
    TEMPORAL_PROPERTIES = {"valid_from", "valid_to", "created_at", "updated_at", "time", "date"}

    def __init__(self, neo4j_conn: Neo4jConnection):
        """Initialize with Neo4j connection"""
        self.neo4j = neo4j_conn

    def batch_upsert_nodes(self, nodes: List[Dict[str, Any]],
    ...
    def batch_upsert_relationships(self, relationships: List[Dict[str, Any]],
                                  batch_size: int = 100) -> Dict[str, Any]:
        ...
        for rel in relationships:
            ...
            if not rel_type:
                results["errors"].append(f"Invalid relationship type: {rel.get('type')}")
                continue
    ...
    def _upsert_relationship_batch(self, rel_type: str,
        ...
        for rel in relationships:
            ...
            rel_data.append({
                "from_id": from_node.get("id") if isinstance(from_node, dict) else from_node,
                "to_id": to_node.get("id") if isinstance(to_node, dict) else to_node,
                "properties": properties
            })
        ...
        try:
            ...
            return {"errors": [f"Batch relationship upsert failed for {rel_type}: {str(e)}"]}

    def upsert_assertion_with_evidence(self, assertion: Dict[str, Any],
                                      chunk_ids: List[str]) -> Dict[str, Any]:
        """Create assertion node with EVIDENCED_BY relationships to chunks"""

        # Create assertion node
        assertion_id = assertion.get("assertion_id") or id_generator.generate_assertion_id(
        ...
        try:
            self.neo4j.execute_query(node_query, {
                "assertion_id": assertion_id,
            ...
            if chunk_ids:
                ...
                self.neo4j.execute_query(evidence_query, {
                    "assertion_id": assertion_id,
                    "chunk_ids": chunk_ids
                })
    ...
    def validate_schema(self, operation: str, data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
